...
class HeartbeatManager implements DatanodeStatistics {
  ...
  synchronized void addDatanode(final DatanodeDescriptor d) {
    ...
    // TODO 往各种数据结构里面存东西
    stats.add(d);
    // TODO 往datanode list结构里面存进去了datanode信息
    datanodes.add(d);
    d.isAlive = true;
  }
  ...
  void heartbeatCheck() {
    ...
    while (!allAlive) {
      ...
      synchronized(this) {
        // TODO 注册的时候，就是把datanode的注册信息放到这个数据结构里面
        // 并且刚刚修改这个datanode上一次心跳信息也是修改这个DatanodeDescriptor数据结构
        // 中datanode信息
        for (DatanodeDescriptor d : datanodes) {
          // TODO 这里判断一个datanode是否dead
          // 什么情况下，50070界面，或者说datanode任务 datanode dead了
